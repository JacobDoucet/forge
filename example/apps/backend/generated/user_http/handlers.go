...
type GetMetadataOnSearchHook func(ctx context.Context, actor permissions.Actor, queryResult user_api.HTTPQueryResult) (map[string]any, error)
type GetMetadataOnCreateHook func(ctx context.Context, actor permissions.Actor, mutationResult user_api.HTTPMutationResult) (map[string]any, error)
type GetMetadataOnUpdateHook func(ctx context.Context, actor permissions.Actor, mutationResult user_api.HTTPMutationResult) (map[string]any, error)
type GetMetadataOnDeleteHook func(ctx context.Context, actor permissions.Actor, deleteResult user_api.HTTPDeleteResult) (map[string]any, error)
...
func (p *HandlerProps) onError(endpoint string, e error) {
	if p.OnError != nil {
		p.OnError(endpoint+"<User>", e)
	}
}

type SearchRequest struct {
	Query      user.HTTPWhereClause `json:"query"`
	Sort       user.HTTPSortParams  `json:"sort"`
	Projection *user_api.Projection `json:"projection,omitempty"`
	Limit      int                  `json:"limit,omitempty"`
	Skip       int                  `json:"skip,omitempty"`
}

func (sr *SearchRequest) ResolveProjection() *user_api.Projection {
	if sr.Projection != nil {
		return sr.Projection
	}
	projection := user_api.NewProjection(true)
...
func GetSearchHandler(props HandlerProps) (http.HandlerFunc, error) {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		queryResult, projectionResult, err := props.Api.Search(ctx, actor, searchQuery, user_api.QueryOptions{
		...
		response, err := user_api.ToHTTPQueryResult(queryResult, projectionResult)
		if err != nil {
			props.onError("Search", err)
...
func (sr *SelectRequest) ResolveProjection() user_api.Projection {
	if sr.Projection != nil {
		return *sr.Projection
	}
	return user_api.NewProjection(true)
}
...
type SelectByIdRequest struct {
	Query user.HTTPSelectByIdQuery `json:"query"`
}
...
func GetSelectByIdHandler(props HandlerProps) (http.HandlerFunc, error) {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		selectByIdRequest := SelectByIdRequest{
			Query: user.HTTPSelectByIdQuery{
				Id: id,
			},
		}
		...
		response, err := user_api.ToHTTPModel(getByIdResult, projectionResult)
		if err != nil {
			props.onError("SelectById", err)
			log.Debug().Err(err).Msg("failed to convert query result to HTTP response")
			http.Error(w, coded_error.ResolveErrorCodeAsString(err), coded_error.ResolveHTTPStatus(err))
...
type SelectByEmailIdxRequest struct {
	Query user.HTTPSelectByEmailIdxQuery `json:"query"`
}

func GetSelectByEmailIdxHandler(props HandlerProps) (http.HandlerFunc, error) {
	if err := props.Validate(); err != nil {
		return nil, err
	}
	return func(w http.ResponseWriter, r *http.Request) {
		selectRequest, err := resolveSelectRequest(r)
		if err != nil {
			props.onError("SelectByEmailIdx", err)
			log.Debug().Err(err).Msg("failed to resolve select request")
			http.Error(w, coded_error.ResolveErrorCodeAsString(err), coded_error.ResolveHTTPStatus(err))
			return
		}
		...
		selectByEmailIdxRequest := SelectByEmailIdxRequest{
			Query: user.HTTPSelectByEmailIdxQuery{
				Email: email,
			},
		}

		selectByEmailIdxQuery, err := selectByEmailIdxRequest.Query.ToSelectByEmailIdxQuery()
		if err != nil {
			props.onError("SelectByEmailIdx", err)
			log.Debug().Err(err).Msg("failed to resolve select query")
			http.Error(w, coded_error.ResolveErrorCodeAsString(err), coded_error.ResolveHTTPStatus(err))
		...
		getByEmailIdxResult, projectionResult, err := props.Api.SelectByEmailIdx(ctx, actor, selectByEmailIdxQuery, projection)
		if err != nil {
			props.onError("SelectByEmailIdx", err)
			log.Debug().Err(err).Msg("failed to execute select query")
			http.Error(w, coded_error.ResolveErrorCodeAsString(err), coded_error.ResolveHTTPStatus(err))
		...
		response, err := user_api.ToHTTPModel(getByEmailIdxResult, projectionResult)
		if err != nil {
			props.onError("SelectByEmailIdx", err)
			log.Debug().Err(err).Msg("failed to convert query result to HTTP response")
			http.Error(w, coded_error.ResolveErrorCodeAsString(err), coded_error.ResolveHTTPStatus(err))
...
type SaveRequest struct {
	Data user.HTTPRecord `json:"data"`
}
...
func GetCreateHandler(props HandlerProps) (http.HandlerFunc, error) {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		response, err := user_api.ToHTTPMutationResult(createResult, projectionResult)
		if err != nil {
			props.onError("Create", err)
...
func GetDeleteHandler(props HandlerProps) (http.HandlerFunc, error) {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		response := user_api.ToHTTPDeleteResult(id)
...
func resolveSearchRequest(r *http.Request) (SearchRequest, error) {
	...
	case http.MethodGet:
		...
		for key, values := range r.URL.Query() {
			...
			case "firstName":
				if len(values) == 1 {
					searchRequest.Query.FirstNameEq = utils.StringSliceToStringPtr(values)
					continue
				}
				if len(values) > 1 {
					searchRequest.Query.FirstNameIn = utils.StringSliceToStringSlicePtr(values)
					continue
				}
			case "language":
				if len(values) == 1 {
					searchRequest.Query.LanguageEq = utils.StringSliceToStringPtr(values)
					continue
				}
				if len(values) > 1 {
					searchRequest.Query.LanguageIn = utils.StringSliceToStringSlicePtr(values)
					continue
				}
			case "lastName":
				if len(values) == 1 {
					searchRequest.Query.LastNameEq = utils.StringSliceToStringPtr(values)
					continue
				}
				if len(values) > 1 {
					searchRequest.Query.LastNameIn = utils.StringSliceToStringSlicePtr(values)
					continue
				}
			}
		}
