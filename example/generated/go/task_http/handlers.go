...
import (
	...
	"github.com/JacobDoucet/forge/example/generated/go/utils"
	"github.com/rs/zerolog/log"
	"io"
	"net/http"
)

type GetMetadataOnSearchHook func(ctx context.Context, actor permissions.Actor, queryResult task_api.HTTPQueryResult) (map[string]any, error)
type GetMetadataOnCreateHook func(ctx context.Context, actor permissions.Actor, mutationResult task_api.HTTPMutationResult) (map[string]any, error)
type GetMetadataOnUpdateHook func(ctx context.Context, actor permissions.Actor, mutationResult task_api.HTTPMutationResult) (map[string]any, error)
type GetMetadataOnDeleteHook func(ctx context.Context, actor permissions.Actor, deleteResult task_api.HTTPDeleteResult) (map[string]any, error)
...
func (p *HandlerProps) onError(endpoint string, e error) {
	if p.OnError != nil {
		p.OnError(endpoint+"<Task>", e)
	}
}

type SearchRequest struct {
	Query      task.HTTPWhereClause `json:"query"`
	Sort       task.HTTPSortParams  `json:"sort"`
	Projection *task_api.Projection `json:"projection,omitempty"`
	Limit      int                  `json:"limit,omitempty"`
	Skip       int                  `json:"skip,omitempty"`
}

func (sr *SearchRequest) ResolveProjection() *task_api.Projection {
...
func GetSearchHandler(props HandlerProps) (http.HandlerFunc, error) {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		queryResult, projectionResult, err := props.Api.Search(ctx, actor, searchQuery, task_api.QueryOptions{
		...
		response, err := task_api.ToHTTPQueryResult(queryResult, projectionResult)
		if err != nil {
			props.onError("Search", err)
...
func (sr *SelectRequest) ResolveProjection() task_api.Projection {
	if sr.Projection != nil {
		return *sr.Projection
	}
	return task_api.NewProjection(true)
}
...
type SelectByIdRequest struct {
	Query task.HTTPSelectByIdQuery `json:"query"`
}
...
func GetSelectByIdHandler(props HandlerProps) (http.HandlerFunc, error) {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		selectByIdRequest := SelectByIdRequest{
			Query: task.HTTPSelectByIdQuery{
				Id: id,
			},
		}
		...
		response, err := task_api.ToHTTPModel(getByIdResult, projectionResult)
		if err != nil {
			props.onError("SelectById", err)
			log.Debug().Err(err).Msg("failed to convert query result to HTTP response")
			http.Error(w, coded_error.ResolveErrorCodeAsString(err), coded_error.ResolveHTTPStatus(err))
...
type SaveRequest struct {
	Data task.HTTPRecord `json:"data"`
}
...
func GetCreateHandler(props HandlerProps) (http.HandlerFunc, error) {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		response, err := task_api.ToHTTPMutationResult(createResult, projectionResult)
		if err != nil {
			props.onError("Create", err)
...
func GetDeleteHandler(props HandlerProps) (http.HandlerFunc, error) {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		response := task_api.ToHTTPDeleteResult(id)
...
func resolveSearchRequest(r *http.Request) (SearchRequest, error) {
	...
	case http.MethodGet:
		...
		for key, values := range r.URL.Query() {
			...
			case "assigneeId":
				if len(values) == 1 {
					searchRequest.Query.AssigneeIdEq = utils.StringSliceToStringPtr(values)
					continue
				}
				if len(values) > 1 {
					searchRequest.Query.AssigneeIdIn = utils.StringSliceToStringSlicePtr(values)
					continue
				}
			case "description":
				if len(values) == 1 {
